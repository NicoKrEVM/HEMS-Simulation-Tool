...
ax1.plot(df["Stunde"], df["PV-Erzeugung"], label="PV-Erzeugung", color="orange", linewidth=2)
ax1.plot(df["Stunde"], df["SOC"], label="Batterie-SOC", color="green", linewidth=2)

# Haushaltsverbrauch & WP-Verbrauch als Balken
bar_width = 0.4
x = np.arange(len(df["Stunde"]))
ax1.bar(x - bar_width/2, df["Haushaltsverbrauch"], width=bar_width, label="Haushaltsverbrauch", color="blue", alpha=0.7)
ax1.bar(x + bar_width/2, df["WP_Optimiert"], width=bar_width, label="WP-Verbrauch", color="red", alpha=0.7)
